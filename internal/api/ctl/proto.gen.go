// forester-controller v0.0.1 6ae8b4e9368f03b6989e6de39daaea74ad46b077
// --
...
func WebRPCSchemaHash() string {
	return "6ae8b4e9368f03b6989e6de39daaea74ad46b077"
}
...
func (s *systemServiceServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/SystemService/BootNetwork":
		s.serveBootNetwork(ctx, w, r)
		return
	case "/rpc/SystemService/BootLocal":
		s.serveBootLocal(ctx, w, r)
		return
	default:
		err := ErrorWithCause(ErrWebrpcBadRoute, fmt.Errorf("no handler for path %q", r.URL.Path))
		RespondWithError(w, err)
...
func (s *systemServiceServer) serveBootNetwork(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveBootNetworkJSON(ctx, w, r)
	default:
		err := ErrorWithCause(ErrWebrpcBadRequest, fmt.Errorf("unexpected Content-Type: %q", r.Header.Get("Content-Type")))
		RespondWithError(w, err)
	}
...
func (s *systemServiceServer) serveBootNetworkJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		err = s.SystemService.BootNetwork(ctx, reqContent.Arg0)
	}()
...
func (s *systemServiceServer) serveBootLocal(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
...
func (s *systemServiceServer) serveBootLocalJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		err = s.SystemService.BootLocal(ctx, reqContent.Arg0)
	}()
